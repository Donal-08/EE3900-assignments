...
\begin{document}
	...
	\begin{figure}[!ht]
		...
		\begin{circuitikz} \draw
			(0,3) to[battery1, l_=$V_1(s)$] (0,0) node[label={below:0}]{} 
				-- (7,0) node[label={below:0}]{}
			(7,3) node[label={above:$V_2$}]{} to[battery1, l=$V_2(s)$] (7,0)
			(3.5,3) node[label={above:$V_c(s)$}] {} to[generic, l=$R_2$] (7,3)
			(3.5,3) to[generic, l_=$R_1$] (0,3) node[label={above:$V_1$}]{}
			(3.5,0) node[ground]{} to[generic, l=$\frac{1}{sC_0}$] (3.5,3)
			;
		\end{circuitikz}
	\end{figure}
	...
	\begin{align} 
        ...
        &= \frac{\frac{1}{R_1} + \frac{2}{R_2}}{\frac{1}{R_1} + \frac{1}{R_2}}\brak{\frac{1}{s} - \frac{1}{\frac{1}{C_0}\brak{\frac{1}{R_1} + \frac{1}{R_2}} + s}} \label{eq:V-s}
	\end{align}
...
\end{align}
\vspace*{10mm} \\
The python code \texttt{codes/2.6.py} plots the graph below.
% \begin{figure}[!htb]
...
\begin{figure}[!ht]
	...
	\begin{circuitikz} \draw
		...
		;
	\end{circuitikz}
	\caption{Circuit diagram in $s$-domain}
	\label{prob:init}
\end{figure}
...
The Python code \texttt{codes/3.4.py} plots the graph below.
% \begin{figure}[!htb]
...
\item Verify your result using ngspice
\solution The ngspice script \texttt{codes/3.5.cir} simulates the given circuit and the generated output is depicted in Fig. \eqref{fig:v2-t}.
