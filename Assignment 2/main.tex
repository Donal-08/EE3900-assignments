...
\end{align}
Also using the definiton in \eqref{eq1}, we can write
\begin{align}
    &e^{j(3\pi n/4)} =  e^{j(3\pi (n+N)/4)} = e^{j(3\pi n/4+2\pi k)} \\
    &\implies 2\pi k = \frac{3\pi}{4}N  \text{,      for integers k, N}\\
    &\implies N = \frac{8}{3}k \text{,      for integers k, N} \\
\end{align}
The smallest k for which both k and  N are integers is 3 , resulting in a period of 8. \\
